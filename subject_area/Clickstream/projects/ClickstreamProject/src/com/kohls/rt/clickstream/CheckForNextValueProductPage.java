 ...
 */
public class CheckForNextValueProductPage extends EvalFunc<DataBag> {
	...
	private static final int THREE=3;//Third element 
	private static final String DWELL_TIME_PRESENT="A";//A when product page contains dwell time
	private static final String DWELL_TIME_ABSENT="D";//D when product page does not contain dwell time
	...
	private static String PRODUCT_PAGE_LAST_DATE_TIME="";//Last date time for last product page
	...
	public DataBag exec(Tuple input) throws IOException {
		...
		for (String key : keys) {
			Tuple tuple = factory.newTuple(THREE);
			if (key.contains(PRODUCT_PAGE_URL_FORMAT)) {
				...
				tuple.set(TWO,DWELL_TIME_PRESENT);//adding third column as Dwell time code 
				resultantTuples.add(tuple);
			} else {
				System.out.println("This is not product page.No need to add to Bag");
			}
		...
		if (LAST_PRODUCT_PAGE.contains(PRODUCT_PAGE_URL_FORMAT)) {
			Tuple lastTuple = factory.newTuple(THREE);
			lastTuple.set(ZERO, LAST_PRODUCT_PAGE);//adding Last product entry
			//Converting milliseconds to time stamp 
			lastTuple.set(ONE, convertInputMillisecondsToTimeStamp(PRODUCT_PAGE_LAST_DATE_TIME));//adding Last product value as LAST_ENTRY
			lastTuple.set(TWO, DWELL_TIME_ABSENT);//adding Last product value as LAST_ENTRY
			resultantTuples.add(lastTuple);//adding last tuple to last of tuples
		} else {
			System.out.println("LAST PAGE : "+LAST_PRODUCT_PAGE +" IS NOT PRODUCT PAGE.SKIPPNG RECORD");
		}
	...
	private static String convertInputMillisecondsToTimeStamp(String input){
