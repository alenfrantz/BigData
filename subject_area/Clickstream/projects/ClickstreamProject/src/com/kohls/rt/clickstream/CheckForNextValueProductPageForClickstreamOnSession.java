 ...
 */
public class CheckForNextValueProductPageForClickstreamOnSession extends EvalFunc<DataBag> {
	...
	private final long SESSION_TIME=1200000;//20 minutes in milliseconds 
	...
	public DataBag exec(Tuple input) throws IOException {
		...
		if (LAST_PRODUCT_PAGE!=null&&!LAST_PRODUCT_PAGE.equals(BLANK) && LAST_PRODUCT_PAGE.contains(PRODUCT_PAGE_URL_FORMAT)) {
			...
			lastTuple.set(ONE, convertInputMillisecondsToEndSessionTimeStamp(PRODUCT_PAGE_LAST_DATE_TIME));
	...
	//function which check whether keys contains value?
	private void checkKeyValue(){
		
	}//end of function which check whether keys contains value?
	//function which converts input milliseconds extracts date part and add 00:20:00 at end of that date 
	private  String convertInputMillisecondsToEndSessionTimeStamp(String input) {
		long inputMilliSeconds=Long.parseLong(input.toString());//get first column 
		long addMiutesToInputTS=inputMilliSeconds+SESSION_TIME;//adding 20 minutes to input
		long difference=addMiutesToInputTS - inputMilliSeconds;
